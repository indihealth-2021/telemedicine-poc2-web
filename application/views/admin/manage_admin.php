  ...
  <div class="content">
      <div class="row mb-4">
          <div class="col-sm-12 col-12 ">
            <nav aria-label="">
                <ol class="breadcrumb" style="background-color: transparent;">
                    ...
                    <li class="breadcrumb-item" aria-current="page"><a href="<?php echo base_url('admin/admin/manage_admin') ?>" class="text-black font-bold-7">User Management</a></li>
                </ol>
            </nav>
          </div>
          <div class="col-sm-12 col-12">
          ...
          <div class="col-sm-12 col-12">
              <h7 class="page-subtitle">Admin</h7>
          </div>
      </div>  
        ...
        <div class="row">
          <div class="col-md-12">
            <!-- <ul class="nav nav-tabs nav-tabs-bottom">
                <li class="nav-item"><a class="nav-link active" href="" data-toggle="tab">Admin</a></li>
            </ul> -->
                ...
                <div class="col-md-12">
                    <div class="box">
                      <div class="container-1">
                          ...
                          <input type="search" id="search" style="background: #ffffff !important;" placeholder="Cari Admin Disini" />
                      </div>
                    </div>
                  <div class="">
                      <a href="<?php echo base_url('admin/Admin/form_admin') ?>" class="btn btn btn-tele btn-rounded float-right  "><i class="fa fa-plus"></i> Tambah Admin</a>
                  </div>
                  <div class="table-responsive pt-4">
                    <table class="table table-border table-hover custom-table mb-0" id="table_manage_admin">
                      <thead class="text-tr">
                      ...
                      <tbody class="font-14">
                        <?php foreach($list_admin as $idx=>$admin){ ?>
                        <tr>
                          ...
                          <td class="text-center"><?php echo $admin->aktif == 1 ? '<span class="status-aktif">Aktif</span>' : '<span class="status-nonaktif">Tidak Aktif</span>' ?></td>
                          <td width="10%"> 
                            <a class="font-icon" href="#modalHapus" data-toggle="modal" data-href="<?php echo base_url('admin/Admin/hapusAdmin/'.$admin->id);?>" data-nama="<?php echo $admin->name ?>" onclick="$('#modalHapus #form')" ><!-- <i class="fas fa-trash fa-xl font-16"></i> -->
                            ...
                            <a class="mx-3" href="<?php echo base_url('admin/Admin/tampilEditAdmin/'.$admin->id);?>"><!-- <i class="fas fa-pen font-16"></i>-->
                              <i><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
...
<div class="modal fade" tabindex="-1" role="dialog" id="modalHapus">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title font-modal-header p-3">Hapus Data Admin</h5>
      </div>
      <form method="post" action="<?php echo base_url('admin/Admin/hapusAdmin/'.$admin->id) ?>">
      <div class="modal-body font-modal-body">
          <p class="p-3">Anda yakin ingin menghapus data admin <b id="nama"></b> ?</p>
      </div>
  ...
  <?php if($this->session->flashdata('error_add_admin')){ ?>
<script>
alert("<?php echo $this->session->flashdata('error_add_admin'); ?>");
</script>
  <?php } ?>
...
<?php echo $this->session->flashdata('msg_hps_admin') ? "<script>alert('".$this->session->flashdata('msg_hps_admin')."')</script>" : ''; ?>
...
<!-- <script>
  $(document).ready(function() {
    ...
    var table = $("#table_manage_admin").dataTable({
      initComplete: function() {
        var api = this.api();
        $('#table_manage_admin_filter input')
          .off('.DT')
      ...
      serverSide: true,
      ajax: {
        "url": "<?php echo base_url() . 'admin/api/admin/get_all_paginate' ?>",
        "type": "POST"
      },
      columns: [
        ...
        },
        {
          ...
          "render": function(data){
            if(data == 1){
              ...
              return "<span class=\"status-nonaktif\">Tidak Aktif</span>";
            }
          }
        },
        {
