  ...
  <div class="content">
      <div class="row mb-4">
          ...
          <div class="col-sm-12 col-12">
              <h3 class="page-title"><?php echo "User Management" ?></h3>
          </div>
          <div class="col-sm-12 col-12">
              <h7 class="page-subtitle">Pengampu dan Diampu</h7>
          </div>
      </div>  
            ...
            <div class="bg-tab px-3">
              <div class="tab-pane show pt-3" id="admin">
                <div class="col-md-12">
                  ...
                  <div class="">
                      <a href="<?php echo base_url('admin/RumahSakit/form_tambah') ?>" class="btn btn btn-tele btn-rounded float-right  "><i class="fa fa-plus"></i> Tambah User</a>
                  </div>
                  <div class="table-responsive pt-4">
                    <table class="table table-border table-hover custom-table mb-0" id="table_users">
                      <thead class="text-tr">
                      ...
                      <tbody class="font-14">
                        <?php foreach($users as $idx => $user_rs){ ?>
                          <tr>
                          <td><?php echo $idx+1?></td>
                          <td><?php echo ucwords($user_rs->name) ?></td>
                          <td><?php echo $user_rs->username ?></td>
                          <td><?php echo $user_rs->email ?></td>
                          <?php $tipe = $user_rs->id_user_level == 1 ? 'Diampu' : 'Pengampu'; ?>
                          <td><?php echo $tipe ?></td>
                          <?php $status = $user_rs->aktif ? '<span class="status-aktif">Aktif</span>' : '<span class="status-nonaktif">Tidak Aktif</span>'; ?>
                          <td><?php echo $status ?></td>
                          <td width="10%"> 
                              <a class="font-icon" href="#modalHapus" data-toggle="modal" data-href="<?php echo base_url('admin/RumahSakit/hapus_user/'.$user_rs->id) ?>" data-nama="<?php echo $user_rs->name ?>" onclick="$('#modalHapus #form')" >
                              ...
                              <a class="mx-3" href="<?php echo base_url('admin/RumahSakit/form_edit_user/'.$user_rs->id) ?>">
                                <i><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title font-modal-header p-3">Hapus Data User</h5>
      </div>
      <form method="post" action="<?php echo base_url('admin/RumahSakit/hapus_user/'.$user_rs->id) ?>">
      <div class="modal-body font-modal-body">
          <p class="p-3">Anda yakin ingin menghapus data user <b id="nama"></b> ?</p>
      </div>
...
<?php echo $this->session->flashdata('msg_user') ? "<script>alert('".$this->session->flashdata('msg_user')."')</script>" : ''; ?>
