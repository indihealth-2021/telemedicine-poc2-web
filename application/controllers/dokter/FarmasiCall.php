...
class FarmasiCall extends CI_Controller
{
    ...
    public function jawab(){
        ...
        $dokter = $this->db->query('SELECT d.name FROM master_user d WHERE d.id_user_kategori = 2 AND d.id = ?', $id_dokter)->row();

        $notifikasi = 'Panggilan dijawab oleh '.$dokter->name;
        ...
        $msg_notif = array(
            'name'=>'accept_panggilan_farmasi_dokter',
            'id_notif'=>$id_notif,
    ...
    public function tolak(){
        ...
        $notifikasi = 'Panggilan ditolak oleh '.$dokter->name;
        $now = (new DateTime('now'))->format('Y-m-d H:i:s');
        $data_notif = array("id_user" => $id_farmasi, "notifikasi" => $notifikasi, "tanggal" => $now, "direct_link" => '#');
        $this->db->insert('data_notifikasi', $data_notif);
        $id_notif = $this->db->insert_id();
        ...
        $msg_notif = array(
            'name'=>'reject_panggilan_farmasi_dokter',
            'id_notif'=>$id_notif,
            'keterangan'=>$notifikasi,
            'tanggal'=>$now,
            'id_user'=>json_encode(array($id_farmasi)),
